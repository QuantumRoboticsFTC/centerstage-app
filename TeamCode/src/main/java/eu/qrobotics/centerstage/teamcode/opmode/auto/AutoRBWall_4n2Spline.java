...
@Config
@Autonomous(name = "01 AutoRBWall 2+6 // Red Backdrop Wall Spline", group = "Red")
public class AutoRBWall_4n2Spline extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        trajectories = TrajectoryRBWall_4n2Spline.getTrajectories(robot, cycleCount, false);
...
//            if (i == 3) pai hai fa baga traiectorie nesimtita stii ce zic la stack 2

            if (i == 1) {
                // 6 initial -> go to stack <-> normal
                robot.drive.followTrajectory(trajectories.get(6));
            } else if (i == 2) {
                // 8 -> go to stack <-> normal
            ...
            if (i <= 2) {
                // 7 -> go to backdrop <-> normal
                robot.drive.followTrajectory(trajectories.get(7));
            } else {
                // 11 -> go to backdrop <-> 2nd stack
